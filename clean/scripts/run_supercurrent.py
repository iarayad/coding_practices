...
phases = np.linspace(0.0, 2.0 * np.pi, 81)
momenta = np.linspace(-np.pi, np.pi, 121)
params = {"mu": 0.4, "delta": 0.2, "t": 1.0}
# %%
result = compute_dispersion_vs_phase(phases=phases, momenta=momenta, params=params, width=20)
...
for target in [0.0, np.pi / 2.0, np.pi]:
    ...
    ax_dispersion.plot(
        ...
        lw=1,
        alpha=0.65,
        label=fr"$\phi={result['phases'][idx]:.2f}$",
    )
